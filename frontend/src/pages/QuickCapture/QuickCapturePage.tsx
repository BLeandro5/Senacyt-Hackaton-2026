...
export default function QuickCapturePage() {
  ...
  return <main className="mx-auto max-w-2xl p-4 sm:p-8"><h1 className="text-3xl font-semibold">Captura rápida</h1><p className="mt-3 text-slate-500">Describe el hospital y los equipos. MedPsy propondrá los datos para tu revisión.</p>
    <textarea aria-label="Nota original" className="my-5 min-h-40 w-full rounded-2xl border border-slate-200 p-4" value={text} disabled={busy || !!analysis} onChange={e => setText(e.target.value)} />
    {!analysis && <button disabled={busy || text.trim().length < 3} className="rounded-xl bg-blue-700 p-3 text-white disabled:opacity-50" onClick={analyze}>{busy ? 'Analizando con MedPsy…' : 'Analizar'}</button>}
