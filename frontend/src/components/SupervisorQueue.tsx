...
export default function SupervisorQueue({ onChange }: { onChange: () => void }) {
  ...
  return <section className="my-6 space-y-4">
    ...
    {data?.hospitals.filter(h => h.verification_status !== 'Confirmed').map(h => <article key={h.id} className="panel">
    ...
    {data?.evidence.map(e => <article key={`${e.visit_id}/${e.observation_id}/${e.id}`} className="panel">
