...
import { nextFollowUp, followUpValue, unknownAnswer } from '../src/data/followUp.ts'
...
test('follow-up targets one device, respects unknown answers and stops at two', () => {
  ...
  assert.equal(nextFollowUp(equipment, ['one:brand']).id, 'two:brand')
  assert.equal(nextFollowUp(equipment, ['one:brand', 'two:brand']), undefined)
