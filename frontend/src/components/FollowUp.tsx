...
export default function FollowUp({ observationId, equipment, update, candidates = [] }: {
  ...
  const respond = (unknown: boolean) => {
    ...
    try {
      ...
      const value = question.field === 'quantityStatus' ? (unknown ? 'unknown' : quantityType) : unknown ? '' : followUpValue(question.field, answer)
      writeStored(key, next)
      if(question.field === 'quantityStatus') equipment.forEach(item=>update(item.id,'quantityStatus',value))
      else update(question.item.id, question.field, value)
  ...
  return <section className="panel mt-5"><p className="font-semibold">Pregunta {answered.length + 1} de 2 · Equipo {question.index + 1}: {question.item.type}</p>
    ...
    <div className="mt-3 flex gap-4"><button disabled={question.field !== 'quantityStatus' && !answer.trim()} className="text-blue-700 disabled:opacity-40" onClick={() => respond(false)}>Guardar respuesta</button><button onClick={() => respond(true)}>No lo sé / Omitir</button></div>
