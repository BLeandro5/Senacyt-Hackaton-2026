...
export default function SettingsPage() {
  const [language, setLanguage] = useState<keyof typeof copy>(() => (localStorage.getItem('language') as keyof typeof copy) in copy ? localStorage.getItem('language') as keyof typeof copy : 'es')
  const [theme, setTheme] = useState(() => localStorage.getItem('appearance') || 'system')
  const [status, setStatus] = useState<{ sqlite: string; ai: { status: string; model?: string; quantization?: string } } | null>(null)
  ...
  return <main className="mx-auto max-w-2xl p-4 sm:p-8"><h1 className="text-3xl font-semibold">{t[0]}</h1><section className="panel mt-5 space-y-5">
    <label className="block">{t[1]}<select className="ml-3 rounded-xl border p-3" value={language} onChange={e => { const value=e.target.value as keyof typeof copy; localStorage.setItem('language',value); setLanguage(value) }}><option value="es">Español</option><option value="en">English</option><option value="pt">Português</option></select></label>
    ...
    <h2 className="text-xl font-semibold">{t[6]}</h2>{status && <><p>SQLite: {status.sqlite}</p><p>{status.ai.model || 'MedPsy'}: {status.ai.status} · {status.ai.quantization || ''}</p><p>QVAC SDK · {t[7]}</p></>}{error && <p role="alert">{error}</p>}
